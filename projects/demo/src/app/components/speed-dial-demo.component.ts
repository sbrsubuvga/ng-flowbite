...
@Component({
  ...
  template: `
    <div>
      ...
      <h1 class="text-4xl font-bold text-white mb-2">Tailwind CSS Speed Dial - Flowbite</h1>
      <p class="text-xl text-gray-400 mb-8">Use the speed dial component to show a floating action button with quick actions.</p>
...
export class SpeedDialDemoComponent {
  speedDialActions = [
    ...
    { label: 'Print', icon: 'print', onClick: () => console.log('Print clicked') },
    { label: 'Download', icon: 'download', onClick: () => console.log('Download clicked') }
  ];

  defaultCode = `<ngf-speed-dial [actions]="speedDialActions" position="bottom-end"></ngf-speed-dial>

speedDialActions = [
  { label: 'Share', icon: 'share', onClick: () => {} },
  { label: 'Print', icon: 'print', onClick: () => {} }
];`;
}
