  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => NgfRadioComponent),
      multi: true
    }
  ]
})
export class NgfRadioComponent implements ControlValueAccessor {
  @Input() label?: string;
  ...
  @Input() radioId = `ngf-radio-${Math.random().toString(36).substr(2, 9)}`;
  @Input() checked = false;

  helperId = `${this.radioId}-helper`;

  private onChangeFn = (value: string | number) => {};
  private onTouchedFn = () => {};

  get radioClasses(): string {
    const base = 'w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600';
    const sizeClasses = {
      sm: 'w-3 h-3',
      md: 'w-4 h-4',
      lg: 'w-5 h-5'
